  ...
  export default {
    ...
    async asyncData ({store, error, params}) {
      await store.dispatch({
        type: native.doSysRoomImgInfo,
        imgId: params.imgId
      }).catch((err, code) => {
        error({message: err, statusCode: code})
      })
      return {imgId: params.imgId}
    },
    data () {
      return {
        img: {
    ...
    created () {
      ...
      this.img = Object.assign({}, this.imgInfo)
    },
    methods: {
      submitForm (formName) {
    ...
    computed: {
      ...mapState({}),
      imgInfo () {
        return this.$store.state.rooms.img[this.imgId]
      }
    }
  }
</script>
