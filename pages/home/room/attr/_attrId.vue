<template>
    <section>
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>修改房间属性</el-breadcrumb-item>
        </el-breadcrumb>
  ...
  export default {
    ...
    async asyncData ({store, error, params}) {
      ...
      await store.dispatch({
        type: native.doSysRoomAttrInfo,
        attrId: params.attrId
      })
      return {attrId: params.attrId}
    },
    data () {
      return {
        attr: {
    ...
    mixins: [attrMixins],
    created () {
      this.attr = Object.assign({}, this.attrInfo)
      this.attr.faces = this.attr.faces.map((face) => face.faceId)
    },
    methods: {
      submitForm (formName) {
    ...
    computed: {
      ...
      attrInfo () {
        return this.$store.state.rooms.attr[this.attrId]
      }
    }
  }
</script>
